	...
	try {
		...
		if (m.isGroup && !groupSet.stay && new Date() * 1 >= groupSet.expired && groupSet.expired != 0) {
			return client
				.reply(
					m.chat,
					Func.texted('italic', '🚩 Bot time has expired and will leave from this group, thank you.', null, {
		...
		if ((body && prefix && commands.includes(command)) || (body && !prefix && commands.includes(command) && setting.noprefix) || (body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command))) {
			...
			for (let name in is_commands) {
				...
				if (!m.isGroup && !['owner', 'menfess', 'scan', 'verify', 'payment', 'premium'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) {
					client
						.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, want to upgrade to premium plan ? send *${prefixes[0]}premium* to see benefit and prices.`, m, {
							largeThumb: true,
			...
			for (let name in is_events) {
				...
				if (!m.isGroup && setting.groupmode && !['system_ev', 'menfess_ev', 'chatbot', 'auto_download'].includes(name) && !isPrem)
					return client
						.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, want to upgrade to premium plan ? send *${prefixes[0]}premium* to see benefit and prices.`, m, {
							largeThumb: true,
				...
				if (event.limit && !event.game && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some((v) => Func.socmed(v)))
					return client.reply(m.chat, `⚠️ You reached the limit and will be reset at 00.00\n\nTo get more limits upgrade to premium plan.`, m).then(() => {
						users.premium = false;
						users.expired = 0;
					});
		...
		if (!m.fromMe || /(undefined|overlimit|timed|timeout|users|item|time)/gi.test(e.message)) {
			let jid = client.decodeJid(m.sender);
			let groupList = async () =>
				Object.entries(await client.groupFetchAllParticipating())
					.slice(0)
			...
			// return m.reply(Func.jsonFormat(client))
			return m.reply('Sepertinya ada masalah! silahkan coba lagi nanti!');
		}
	}
