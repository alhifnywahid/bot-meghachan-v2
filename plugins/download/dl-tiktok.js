  ...
  async: async (m, { client, args, isPrefix, command, Func }) => {
    try {
      if (!args || !args[0])
        return client.reply(
          m.chat,
          Func.example(isPrefix, command, "https://vm.tiktok.com/ZSR7c5G6y/"),
          m
        );
      ...
      let json = await Func.fetchJson(
        `https://dikaardnt.com/api/download/tiktok?url=${args[0]}`
      );
      ...
      if (command == "tiktok" || command == "tt") {
        ...
        if (json.video)
          return client.sendFile(
            m.chat,
            json.video.url.without_watermark,
            "video.mp4",
            quotes,
            m
          );
