...
exports.run = {
  ...
  async: async (
    m,
    { client, args, isPrefix, command, users, env, Func, Scraper }
  ) => {
    try {
      ...
      if (/get?(mp4|mp3)/.test(command) && check && !isNaN(args[0])) {
        if (Number(args[0]) > check.results.length)
          return m.reply(Func.texted("bold", `🚩 Exceed amount of data.`));
        client.sendReact(m.chat, "🕒", m.key);
        if (command === "getmp3") {
          const json = await Scraper.youtube(
            check.results[Number(args[0]) - 1]
          );
          if (!json.status)
          ...
          const isOver = users.premium
            ...
            : `⚠️ File size (${json.data.size}), you can only download files with a maximum size of ${env.max_upload_free} MB and for premium users a maximum of ${env.max_upload} MB.`;
        ...
        const json = await Func.fetchJson(
          `https://dikaardnt.com/api/youtube/playlist?url=${args[0]}`
        );
