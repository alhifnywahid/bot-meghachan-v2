	...
	async: async (m, { message, client, text, isPrefix, command, setting, users, plugins, env, Func }) => {
		try {
			...
			const message = setting.msg
				...
				.replace('+db', /mongo/.test(process.env.DATABASE_URL) ? 'Mongo' : /postgre/.test(process.env.DATABASE_URL) ? 'Postgres' : `Local (${local_size})`)
				.replace('+version', (library.devDependencies.bails ? library.devDependencies.bails : library.devDependencies['@adiwajshing/baileys'] ? '@adiwajshing/baileys' : library.devDependencies.baileys).replace('^', '').replace('~', ''));
