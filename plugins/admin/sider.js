exports.run = {
	...
	async: async (m, { message, client, args, isPrefix, command, participants, isBotAdmin, Func }) => {
		try {
			...
			var lastseen = Object.entries(global.db.groups.find((v) => v.jid == m.chat).member)
				.filter(([jid, data]) => data.lastseen)
				.sort((a, b) => a[1].lastseen - b[1].lastseen)
				.filter(([v, x]) => x.lastseen != 0 && (now - x.lastseen > day || now - global.db.users.find((c) => c.jid == v).lastseen > day) && global.db.users.some((c) => c.jid == v) && !global.db.users.find((c) => c.jid == v).premium && !global.db.users.find((c) => c.jid == v).whitelist && v != client.decodeJid(client.user.id));
			if (args && args[0] == '-y') {
				if (!isBotAdmin) return client.reply(m.chat, global.status.botAdmin, m);
				let arr = Object.entries(lastseen)
					.map(([jid, _]) => jid)
					.concat(sider);
				...
				if (sider.length == 0 && lastseen.length == 0) return client.reply(m.chat, Func.texted('bold', `🚩 There is no sider in this group.`), m);
